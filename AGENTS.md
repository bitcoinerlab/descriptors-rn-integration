...
- All three `@bitcoinerlab/*` dependencies are local tarballs at exact sibling
  paths. After a library change, run `(cd ../bitbox-react-native && npm pack)`;
  after descriptor changes, run these commands in order:
...
- After dependency/plugin changes run `npx expo prebuild --clean`, inspect both
  ...
  the provider/platform matrix in `README.md`; Ledger remains unvalidated until
  that physical matrix is completed.
